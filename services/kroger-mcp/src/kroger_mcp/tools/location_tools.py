...
def register_tools(mcp):
    ...
    async def search_locations(
        ...
        try:
            locations = client.location.search_locations(
                zip_code=zip_code, radius_in_miles=radius_in_miles, limit=limit, chain=chain
            )

            if not locations or "data" not in locations or not locations["data"]:
                return {"success": False, "message": f"No locations found near zip code {zip_code}", "data": []}
    ...
    @mcp.tool()
    async def get_location_details(location_id: str, ctx: Context = None) -> dict[str, Any]:
        """
    ...
    @mcp.tool()
    async def check_location_exists(location_id: str, ctx: Context = None) -> dict[str, Any]:
        """
