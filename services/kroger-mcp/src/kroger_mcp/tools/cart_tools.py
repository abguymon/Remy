...
def register_tools(mcp):
    ...
    async def remove_from_cart(product_id: str, modality: str = None, ctx: Context = None) -> dict[str, Any]:
        ...
        try:
            ...
            if modality:
                ...
                cart_data["current_cart"] = [item for item in current_cart if item.get("product_id") != product_id]
    ...
    async def mark_order_placed(order_notes: str = None, ctx: Context = None) -> dict[str, Any]:
        ...
        try:
            ...
            if not current_cart:
                return {"success": False, "error": "No items in current cart to mark as placed"}
    ...
    async def view_order_history(limit: int = 10, ctx: Context = None) -> dict[str, Any]:
        ...
        try:
            ...
            return {"success": False, "error": f"Failed to view order history: {str(e)}"}
