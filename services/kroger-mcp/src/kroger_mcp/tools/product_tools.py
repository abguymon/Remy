...
from .shared import format_currency, get_client_credentials_client, get_preferred_location_id
...
def register_tools(mcp):
    ...
    async def get_product_images(
        ...
        try:
            ...
            product_details = client.product.get_product(product_id=product_id, location_id=location_id)
    ...
    async def search_products(
        ...
        try:
            products = client.product.search_products(
                term=search_term, location_id=location_id, limit=limit, fulfillment=fulfillment, brand=brand
            )

            if not products or "data" not in products or not products["data"]:
                return {"success": False, "message": f"No products found matching '{search_term}'", "data": []}
        ...
        try:
            ...
            if "items" in product and product["items"]:
                ...
                if "price" in item:
                    ...
                    result["pricing"] = {
                        ...
                        "on_sale": price.get("promo") is not None
                        and price.get("promo") < price.get("regular", float("inf")),
                        "savings": price.get("regular", 0) - price.get("promo", price.get("regular", 0))
                        if price.get("promo")
                        else 0,
                    }
            ...
            if "images" in product and product["images"]:
                result["images"] = [
                    {
                        "perspective": img.get("perspective"),
                        "sizes": [{"size": size.get("size"), "url": size.get("url")} for size in img.get("sizes", [])],
                    }
                    for img in product["images"]
                ]
    ...
    async def search_products_by_id(
        ...
        try:
            products = client.product.search_products(product_id=product_id, location_id=location_id)
            ...
            return {
                "success": True,
                "search_params": {"product_id": product_id, "location_id": location_id},
                "count": len(formatted_products),
                "data": formatted_products,
            }
