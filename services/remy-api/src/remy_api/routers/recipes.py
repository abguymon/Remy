...
async def create_recipe_from_url(payload: RecipeFromUrl, user: CurrentUser, session: SessionDep) -> RecipeDetail:
    # TODO(T5/orchestrator): pass T4's LLM client as ``llm=`` to enable the
    # structured-output fallback for pages recipe-scrapers can't handle. Until
    # wired, only the scraper path runs and unparseable pages raise a typed
    # RecipeParseError (422) listing what failed.
    parsed = await scrape_recipe(payload.url, llm=None)
