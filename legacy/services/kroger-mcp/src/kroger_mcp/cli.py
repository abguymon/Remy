...
def main():
    ...
    parser = argparse.ArgumentParser(description="Kroger MCP Server - FastMCP server for Kroger API integration")

    parser.add_argument("--client-id", help="Kroger API client ID (can also use KROGER_CLIENT_ID env var)")
    parser.add_argument("--client-secret", help="Kroger API client secret (can also use KROGER_CLIENT_SECRET env var)")
    parser.add_argument(
        "--redirect-uri",
        default="http://localhost:8000/callback",
        help="OAuth redirect URI (default: http://localhost:8000/callback)",
    )
    parser.add_argument("--zip-code", help="Default zip code for location searches")
    parser.add_argument(
        "--transport",
