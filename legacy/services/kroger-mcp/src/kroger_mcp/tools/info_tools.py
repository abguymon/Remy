...
def register_tools(mcp):
    ...
    async def list_chains(ctx: Context = None) -> dict[str, Any]:
        ...
        try:
            ...
            # Format chain data
            formatted_chains = [
                {"name": chain.get("name"), "division_numbers": chain.get("divisionNumbers", [])}
            ...
            return {"success": True, "count": len(formatted_chains), "data": formatted_chains}
    ...
    @mcp.tool()
    async def get_chain_details(chain_name: str, ctx: Context = None) -> dict[str, Any]:
        """
        ...
        try:
            ...
            return {"success": True, "name": chain.get("name"), "division_numbers": chain.get("divisionNumbers", [])}
    ...
    @mcp.tool()
    async def check_chain_exists(chain_name: str, ctx: Context = None) -> dict[str, Any]:
        """
    ...
    async def list_departments(ctx: Context = None) -> dict[str, Any]:
        ...
        try:
            ...
            formatted_departments = [
                {"department_id": dept.get("departmentId"), "name": dept.get("name")} for dept in departments["data"]
            ]

            if ctx:
                await ctx.info(f"Found {len(formatted_departments)} departments")

            return {"success": True, "count": len(formatted_departments), "data": formatted_departments}
    ...
    @mcp.tool()
    async def get_department_details(department_id: str, ctx: Context = None) -> dict[str, Any]:
        """
        ...
        try:
            ...
            if not dept_details or "data" not in dept_details:
                return {"success": False, "message": f"Department '{department_id}' not found"}

            dept = dept_details["data"]

            return {"success": True, "department_id": dept.get("departmentId"), "name": dept.get("name")}
    ...
    @mcp.tool()
    async def check_department_exists(department_id: str, ctx: Context = None) -> dict[str, Any]:
        """
